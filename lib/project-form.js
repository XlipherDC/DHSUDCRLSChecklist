...
export function bindProjectFields(form) {
  ...
  const update = () => {
    ...
    const condo = form.querySelector('#condominium-fields'); condo.hidden = condo.disabled = subdivision;
  };
...
export function readProjectFields(form, type) {
  ...
  return {
    ...(type === 'subdivision' ? { categories: data.getAll('categories'), scheme: categoryScheme(data.getAll('categories')[0]), lotCounts: { houseAndLot: Number(data.get('houseAndLot')), lotOnly: Number(data.get('lotOnly')) } } : {}),
