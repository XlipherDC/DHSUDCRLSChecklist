import { validateSubmittedPlans } from './plan-checklist.js';
import { prepareAssessment, calculateAssessment, validateAssessmentInputs, validateAssessment } from './fee-schedule.js';
...
export function validateFeeInputs(fees) {
  if (fees.version !== undefined) return validateAssessmentInputs(fees);
...
export function calculateFees(project, catalog) {
  if (project.fees.version !== undefined) return calculateAssessment(project);
  return calculateLegacyFees(project, catalog);
}
export function calculateLegacyFees(project, catalog) {
...
export function validateProject(p, catalog) {
  ...
  for (const [id, review] of Object.entries(p.reviews)) {
    ...
    validateSubmittedPlans(catalog[p.type].requirements.find(requirement => requirement.id === id), review);
  ...
  if (p.fees.version !== undefined) validateAssessment(p, p.fees.saved);
