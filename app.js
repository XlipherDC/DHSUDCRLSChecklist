...
function requirementList(p) {
  ...
  return list.map(r => { const review = p.reviews[r.id], status = review?.status || 'missing'; let header = ''; if (group !== r.category) { group = r.category; header = `<div class="requirement-group">${esc(group)}</div>`; } return `${header}<button class="requirement-row" data-action="review" data-id="${r.id}"><span class="requirement-mark ${status}">${icon(status === 'verified' ? 'check' : status === 'submitted' ? 'clock' : 'file')}</span><span class="requirement-text"><strong>${esc(r.title)}</strong><small>${esc(r.source)}${review?.remarks ? ` <span>· ${esc(review.remarks.slice(0, 100))}</span>` : ''}</small></span>${badge(STATUSES[status], status)}${icon('arrow')}</button>`; }).join('');
}
function detailsView(p) {
  const details = [['Project name', p.name], ['Project reference', p.reference], ['Project type', catalog[p.type].label], ['Category', SCHEMES[p.scheme]], ['Location', p.location], ['Project area', p.projectArea ? `${number(p.projectArea)} m²` : ''], ['Owner', p.owner], ['Developer', p.developer], ['Developer address', p.address], ['Application', p.application], ['Processor', p.processor], ['Date received', p.receivedDate ? date(p.receivedDate) : '']];
  return `<section class="panel"><div class="panel-heading"><div><h2>Project information</h2><p>Details used throughout this project’s review and fee assessment.</p></div>${button('edit', 'Edit details', 'edit')}</div><dl class="details-grid">${details.map(([label, value]) => `<div><dt>${label}</dt><dd>${esc(value || 'Not entered')}</dd></div>`).join('')}</dl><div class="notes-block"><h3>Project notes</h3><p class="pre-line">${esc(p.notes || 'No project notes yet.')}</p></div></section><section class="panel source-strip"><div>${icon('file')}<div><h3>Checklist source</h3><p>${esc(catalog[p.type].source)}</p></div></div>${button('sources', 'View source notes', 'arrow', 'btn text-btn')}</section><div class="danger-zone"><span>Created ${date(p.createdAt)} · Last updated ${date(p.updatedAt)}</span>${button('delete-project', 'Delete project', 'trash', 'btn danger-ghost')}</div>`;
}
function feesView(p) {
  const f = p.fees, condo = p.type === 'condominium' && p.scheme === 'pd957';
  const qty = (label, name, unit, step = '1') => `<label class="field"><span>${label}</span><div class="input-unit"><input name="${name}" type="number" min="0" max="10000000000" step="${step}" value="${f[name]}" required><span>${unit}</span></div></label>`;
  return `<div class="notice">${icon('info')}<div><strong>Assessment based on the supplied workbook rates</strong><p>These rates have no verified effective date. Confirm the applicable schedule before issuing a payment order. Inspection follows the source’s “per lot” label; enter the assessed lot count explicitly.</p></div></div><form id="fees-form"><div class="fee-layout"><section class="panel fee-inputs"><div class="panel-heading"><div><h2>Assessment inputs</h2><p>${esc(SCHEMES[p.scheme])}</p></div>${icon('calc')}</div><div class="form-body"><h3>License to Sell</h3><div class="form-grid">${condo ? qty('Residential saleable area', 'residentialArea', 'm²', '0.01') + qty('Commercial saleable area', 'commercialArea', 'm²', '0.01') : qty('Number of lots', 'lots', 'lots') + qty('Total housing floor area', 'housingArea', 'm²', '0.01')}</div><h3>Registration & other charges</h3><label class="checkbox-field"><input type="checkbox" name="includeRegistration" ${f.includeRegistration ? 'checked' : ''}><span>Include Certificate of Registration fee</span></label><div class="form-grid">${qty('CR forms', 'crForms', 'forms')}${qty('LS forms', 'lsForms', 'forms')}${qty('Lots subject to inspection', 'inspectionLots', 'lots')}${qty('SH provisional certificates', 'provisionalForms', 'forms')}</div><h3>Assessment record</h3><div class="form-grid">${field('Prepared by', 'preparedBy', f.preparedBy || p.processor)}${field('O.R. / payment reference', 'receipt', f.receipt)}${qty('Amount already paid', 'paid', 'PHP', '0.01')}</div><label class="field"><span>Assessment notes / basis for charges</span><textarea name="notes" rows="3" maxlength="10000">${esc(f.notes)}</textarea></label><p class="muted small">Zero quantities contribute no fee. CR/LS form quantities are editable for the application being assessed.</p></div></section><aside class="fee-result"><section class="panel"><div class="panel-heading"><div><p class="eyebrow">FEE BREAKDOWN</p><h2>Estimated assessment</h2></div><span id="assessment-state" class="type-tag">${f.saved ? 'Saved' : 'Draft'}</span></div><div id="fee-breakdown">${feeBreakdown(p)}</div><div class="fee-actions"><button class="btn primary full" type="submit">${icon('check')}Save assessment</button><p class="muted small">Save to include this assessment in the project summary.</p></div></section><p class="source-caption">Source: ${esc(catalog[p.type].source)} · Fee worksheet. Each line below shows its rate cell.</p></aside></div></form>`;
}
function feeBreakdown(p) {
  const calc = calculateFees(p, catalog);
  return `<div class="fee-lines">${calc.lines.map(l => `<div class="fee-line"><div><strong>${l.label}</strong><small>${number(l.quantity)} ${l.unit} × ${money(l.rate)} <span>· ${l.source}</span></small></div><b>${money(l.amount)}</b></div>`).join('')}</div><div class="fee-total"><span>Total assessed fees</span><strong>${money(calc.total)}</strong><small>Philippine pesos · Based on selected charges</small></div><div class="fee-payment"><div><span>Amount paid</span><b>${money(p.fees.paid)}</b></div><div><span>Balance due</span><b>${money(calc.balance)}</b></div>${calc.overpayment ? `<div><span>Overpayment</span><b>${money(calc.overpayment)}</b></div>` : ''}</div>`;
}
function sourcesView() {
  return `<div class="page-heading"><div><p class="eyebrow">REFERENCE LIBRARY</p><h1>Checklist sources</h1><p>Requirements and fee rates extracted from your original workbooks.</p></div></div><div class="source-cards">${Object.entries(catalog).map(([type, c]) => `<section class="panel source-card"><span class="empty-icon">${icon('building')}</span><h2>${c.label}</h2><p>${c.requirements.length} document requirements</p><code>${esc(c.source)}</code><div class="source-categories">${[...new Set(c.requirements.map(r => r.category))].map(cat => `<span>${icon('check')}${esc(cat)}</span>`).join('')}</div>${button(`new-${type}`, `Create ${type} project`, 'plus', 'btn primary')}</section>`).join('')}</div><section class="panel prose"><h2>How the source workbooks are used</h2><p>The app preserves the main document requirements and their nested review prompts. Every requirement and fee rate includes its worksheet and cell reference. New projects begin with unreviewed requirements; existing source annotations, signatures, and sample project details are not copied.</p><h3>Fee calculation corrections</h3><ul><li>The subdivision workbook’s Fee!L21 sums PD957 and both BP220 schedules together. This app includes only the selected category.</li><li>The condominium workbook’s Fee!L29 omits the commercial saleable area charge at L14. This app includes residential and commercial charges under PD957.</li><li>Inspection is explicitly entered as a number of lots, matching the worksheet label. It is not automatically charged once per project.</li><li>The workbooks contain separate socialized and economic housing rates. Mixed-category projects need separate assessment records.</li></ul><h3>Review scope</h3><p>Supporting Titles, Design Standard, and Area Tab worksheets are not automatic compliance decisions. Record findings from reviewed supporting sheets in the corresponding requirement’s remarks. Verification indicates a processor’s document review, not regulatory approval.</p><h3>Rate currency</h3><p>The supplied fee sheets do not establish an effective date. Rates are reproduced as reference inputs, not certified as the current fee schedule. Confirm applicable rates before issuing an official assessment.</p></section>`;
}
...
function projectForm(type = 'subdivision', edit = false) {
  const p = edit ? current() : { type, scheme: 'pd957' };
  openModal(`${modalHeader(edit ? 'PROJECT SETTINGS' : 'NEW PROJECT', edit ? 'Edit project details' : 'Start a project review')}<form id="project-form"><div class="modal-body"><div class="form-grid"><label class="field"><span>Project type</span><select name="type" ${edit ? 'disabled' : ''}>${options({ subdivision: 'Subdivision', condominium: 'Condominium' }, p.type)}</select></label><label class="field"><span>Project category</span><select name="scheme">${options(SCHEMES, p.scheme)}</select></label></div>${edit ? '<p class="small muted">Project type is fixed to preserve checklist reviews. Changing category resets the saved fee assessment to a draft.</p>' : '<p class="small muted">The selected source checklist is added automatically. Mark conditional requirements N/A with a reason.</p>'}${field('Project name <span class="required">*</span>', 'name', p.name, 'text', 'required maxlength="200" placeholder="e.g. Riverside Residences"')}<div class="form-grid">${field('Project reference', 'reference', p.reference, 'text', 'maxlength="200" placeholder="e.g. CRLS-2026-001"')}${field('Date received', 'receivedDate', p.receivedDate, 'date')}${field('Location', 'location', p.location, 'text', 'maxlength="1000"')}${field('Project land area (m²)', 'projectArea', p.projectArea, 'number', 'min="0" step="0.01"')}${field('Owner', 'owner', p.owner, 'text', 'maxlength="1000"')}${field('Developer', 'developer', p.developer, 'text', 'maxlength="1000"')}</div>${field('Developer address', 'address', p.address, 'text', 'maxlength="1000"')}<div class="form-grid">${field('Application', 'application', p.application || 'Certificate of Registration and License to Sell', 'text', 'maxlength="1000"')}${field('Assigned processor', 'processor', p.processor, 'text', 'maxlength="200"')}</div><label class="field"><span>Project notes</span><textarea name="notes" rows="3" maxlength="10000">${esc(p.notes)}</textarea></label><p class="form-error" role="alert"></p></div><div class="modal-footer">${button('close-modal', 'Cancel', null)}<button class="btn primary" type="submit">${icon(edit ? 'check' : 'plus')}${edit ? 'Save project' : 'Create project'}</button></div></form>`);
  ...
  form.addEventListener('submit', async e => { e.preventDefault(); if (saving) return; const submit = $('button[type=submit]', form); submit.disabled = saving = true; try {
    ...
    let record;
    if (edit) { record = structuredClone(p); Object.assign(record, values, { projectArea: Number(values.projectArea || 0), name: values.name.trim(), updatedAt: new Date().toISOString() }); if (record.scheme !== p.scheme) record.fees.saved = false; }
...
function feeDraft(form) {
  const f = { ...current().fees, saved: true };
  const values = new FormData(form);
  for (const key of ['lots', 'housingArea', 'residentialArea', 'commercialArea', 'inspectionLots', 'crForms', 'lsForms', 'provisionalForms', 'paid']) if (values.has(key)) f[key] = Number(values.get(key));
  for (const key of ['preparedBy', 'receipt', 'notes']) f[key] = values.get(key);
  f.includeRegistration = values.has('includeRegistration');
  return { ...current(), fees: f, updatedAt: new Date().toISOString() };
}
function bindView() {
  bindSharing();
  ...
  fees?.addEventListener('input', () => { dirty = true; $('#assessment-state').textContent = 'Unsaved'; try { if (!fees.checkValidity()) { $('#fee-breakdown').innerHTML = '<p class="form-error padded">Enter valid, non-negative quantities to calculate fees.</p>'; return; } $('#fee-breakdown').innerHTML = feeBreakdown(feeDraft(fees)); } catch (err) { $('#fee-breakdown').innerHTML = `<p class="form-error padded">${esc(err.message)}</p>`; } });
...
async function handleAction(btn) {
  ...
  if (['projects', 'sources', 'backups', 'sharing'].includes(action)) return navigate(action);
...
function printProject() {
  ...
  report.innerHTML = `<div class="print-header"><img src="./favicon.svg" width="42" alt=""><div><h1>CRLS Project Review Summary</h1><p>Certificate of Registration & License to Sell · Printed ${date(new Date().toISOString())}</p></div></div><h2>${esc(p.name)}</h2><p>${esc(p.location)}</p><dl class="print-details">${[['Reference', p.reference], ['Type', catalog[p.type].label], ['Category', SCHEMES[p.scheme]], ['Owner', p.owner], ['Developer', p.developer], ['Address', p.address], ['Application', p.application], ['Land area', `${number(p.projectArea)} m²`], ['Processor', p.processor], ['Date received', date(p.receivedDate)]].map(([k, v]) => `<div><dt>${k}</dt><dd>${esc(v || '—')}</dd></div>`).join('')}</dl>${p.notes ? `<p><b>Project notes:</b> ${esc(p.notes)}</p>` : ''}<h3>Document review · ${summary.verified} / ${summary.applicable} verified · ${summary.na} not applicable</h3><table class="print-table"><thead><tr><th>Requirement / source</th><th>Status</th><th>Review details</th></tr></thead><tbody>${catalog[p.type].requirements.map(r => { const review = p.reviews[r.id]; return `<tr><td><strong>${esc(r.title)}</strong><small>${esc(r.source)}</small></td><td>${STATUSES[review?.status || 'missing']}</td><td><span class="pre-line">${esc(review?.remarks || '—')}</span>${review ? `<small>Received: ${date(review.receivedDate)} · Reviewer: ${esc(review.reviewedBy || '—')}</small>` : ''}</td></tr>`; }).join('')}</tbody></table><section class="print-fees"><h2>Fee assessment</h2>${p.fees.saved ? feeBreakdown(p) + `<p>Prepared by: ${esc(p.fees.preparedBy || '—')} · O.R. / payment reference: ${esc(p.fees.receipt || '—')}</p><p class="pre-line">${esc(p.fees.notes)}</p>` : '<p>No fee assessment has been saved.</p>'}<p class="small">Rates from ${esc(catalog[p.type].source)}, Fee sheet. Confirm the applicable schedule before issuing an official assessment. Review completion is not regulatory approval.</p></section>`;
...
window.addEventListener('hashchange', () => {
  ...
  else { activeId = undefined; page = ['sources', 'backups', 'sharing'].includes(route) ? route : 'projects'; }
  shell();
});
async function init() {
  ...
  else if (['sources', 'backups', 'sharing'].includes(route)) page = route;
  shell();
}
