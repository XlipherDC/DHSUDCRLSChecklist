...
function projectForm(type = 'subdivision', edit = false) {
  ...
  form.addEventListener('submit', async e => { e.preventDefault(); if (saving) return; const submit = $('button[type=submit]', form); submit.disabled = saving = true; try {
    ...
    let record;
    if (edit) { record = structuredClone(p); Object.assign(record, values, { projectArea: Number(values.projectArea || 0), name: values.name.trim(), updatedAt: new Date().toISOString() }); if (record.scheme !== p.scheme || JSON.stringify(record.categories) !== JSON.stringify(p.categories)) record.fees.saved = false; }
